...
2. Press `Delete` + `Left-click` on the target item.
3. Confirm the action with `Enter` or `Space`.
...
* **Technical Note:** To equip a stackable crystal, only one may remain in the inventory. Store extras in **City Storage** or **Market Storage** before equipping to avoid system errors.
...
* Use a **Processing Stone** to enable **Mass Processing** for these recipes.
